...
def _one_sided_result(
    ...
    Порядок веток важен: сначала «пусты обе стороны», затем «проводку
    сторнировали», и только потом «проводки не было вовсе».
    """
    unpaired_acc = _sum(postings)
    unpaired_ops = _sum(operations)

    # Пусты **обе** стороны: у каждой проводки дня нашлась выдача и у каждой
    # выдачи — проводка, только партнёр лежит на соседней дате. Сопоставление
    # §5.9.2 идёт в окне ``DATE_WIN`` по всему периоду (иначе подокументная
    # инкассация не сводится вовсе), поэтому дневная разница при полностью
    # разобранном дне — это сдвиг дат, а не расхождение.
    #
    # Пока ветки не было, такой день падал в subset-sum с пустым списком
    # кандидатов, гейты §7.3 закрывались «подмножество не найдено», и день
    # получал отказ с ненулевым ``balance_impact``. На Максиму_касса_2 таких
    # дней 638 — при том что §11.3 говорит об этой кассе «дискретных ошибок
    # нет». Отказ «не локализовано» там, где локализовать нечего, — не
    # осторожность, а ложная тревога.
    if not postings and not operations:
        return _result(
            day,
            category,
            mode,
            ...
            _ZERO,
            (
                f"За {day:%d.%m.%Y} каждая проводка сведена с выдачей и каждая "
                f"выдача — с проводкой в окне DATE_WIN (§5.9.2).{paired_note} "
                "Дневная разница объясняется датировкой документов, а не "
                "расхождением; на сальдо не влияет."
            ),
        )
...
def _churn_refusal(
    ...
    Отказ касается **документа, а не величины**: нетто категории известно из
    §5.6 точно, и ``balance_impact`` несёт именно его. Пока здесь стоял ноль,
    вклад категории в отклонение не имел носителя и молча оседал в строке «не
    локализовано»: на Щучине так пропадали +8 301,10 инкассации, которые §11.3
    называет таймингом, а на `Кса_с_отклонением` — краевой артефакт +4 038,06.
    """
    ...
    return _result(
        ...
        None,
        abs(recon.net),
        balance_impact_of(category, recon.net),
        (
