...
class CategoryRecon(_Frozen):
    ...
    **Какой ряд считается сверяемым.** ``ops_series`` — вариант «как есть»,
    ...
    вовсе: сверять не с чем, и разности дней не строятся. На
    `Кса_с_отклонением` и `Максиму_касса_2` блок ПКО отсутствует, и без этого
    флага весь приход (2 979 052,00 и 4 870 682,41) объявлялся расхождением —
    810 и 1 042 «проблемных дня» на пустом месте.
    """
